...
const CustomCarouselServices = ({ items, title }) => {
                ...
                <div className="custom_carousel-items row-centered" ref={carouselRef}>
                    {items.map((item, index) => (
                        <div key={index} className="custom_carousel-item column-centered">
                            ...
                            <span className="custom_carousel_card-text">{item.name || ''} - R$ { item.price.toFixed(2) || ''}</span>
                        </div>
                    ))}
                </div>
...
export default CustomCarouselServices;